...
it('will return the translation for the fallback locale if the current locale does not exist', function () {
    ...
    setCurrentLocale('fr');
    setFallbackLocale('en');
    expect($translator->getCustomTranslation(['en' => 'test en', 'de' => 'test de']))->toBe('test en');
});

it('will return the missing translation string if no translation is found', function () {
    $translator = new CustomTranslator();
    expect($translator->getCustomTranslation(['hu' => 'test']))->toBe(config('site.translations.missing-translation'));
});

it('will return the translations string for any locale if the translations are a string', function () {
    $translator = new CustomTranslator();
    expect($translator->getCustomTranslation('test'))->toBe('test')
        ->and($translator->getCustomTranslation('test', 'de'))->toBe('test')
        ->and($translator->getCustomTranslation('test', 'hu'))->toBe('test');
});
...
it('will set the translation for the requested locale', function () {
    ...
    expect(currentLocale())->toBe('en')
        ->and($translator->setCustomTranslation('test', 'de', 'test de'))->toBe(['en' => 'test', 'de' => 'test de']);
});

it('will set the translation for the requested locale if the translations are already an array', function () {
    $translator = new CustomTranslator();
    expect($translator->setCustomTranslation(['en' => 'test'], 'de', 'test de'))->toBe(['en' => 'test', 'de' => 'test de']);
});
